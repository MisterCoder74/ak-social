...
if($u == $log_username && $user_ok == true){
	...
	$photo_form .=   '</select>';
        $photo_form .=   ' <br><br><b>Choose Photo:</b> ';
	$photo_form .=   '<label class="custom-file-upload"><input type="file" name="photo" required />Select</label>';
        $photo_form .=   '<br><br><h4>Insert a description for the photo</h4><textarea name="description" style="overflow:auto;resize:none" rows="5" cols="54"></textarea><br>';
...
if(mysqli_num_rows($query) < 1){
	...
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		$count = $countrow[0];
		$filequery = mysqli_query($db_conx, "SELECT filename, description FROM photos WHERE user='$u' AND gallery='$gallery'");
      		$filerow = mysqli_fetch_row($filequery);
		$file = $filerow[0];
                		$description = $filerow[1];
		$gallery_list .= '<div>';
...
<style type="text/css">
form#photo_form{background:#fff; border:black 1px solid; padding:20px; box-shadow: 0px 2px 6px grey;}
div#galleries{}
...
div#photos{display:none; border:black 1px solid; padding:20px;}
...
function photoShowcase(picdata){
	...
	_("picbox").innerHTML += '<img src="user/<?php echo $u; ?>/'+data[1]+'" alt="photo"><br><?php echo $description; ?><br><form id="like_system" enctype="multipart/form-data" method="post" action="like_system.php"><button type="submit" id="like" name="like" disabled>Like</button> <button type="submit" id="dislike" name="dislike" disabled>Dislike</button></form>';
