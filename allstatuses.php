...
$statuslist = "";
if($isOwner == "yes"){
	$status_ui = '<textarea id="statustext" onkeyup="statusMax(this,550)" placeholder="What&#39;s new with you '.$u.'?"></textarea><br>';
	$status_ui .= '<button id="statusBtn" onclick="postToStatus(\'status_post\',\'a\',\''.$u.'\',\'statustext\')">Post</button><br><br><hr><br>';
	...
	$status_ui .= '<button id="statusBtn" onclick="postToStatus(\'status_post\',\'c\',\''.$u.'\',\'statustext\')">Post</button><br><br><hr><br>';
}
?><?php 
$sql = "SELECT * FROM status WHERE type='a' OR type = 'c' ORDER BY postdate DESC LIMIT 20";
...
while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
	...
	if($isFriend == true || $log_username == $u){
	    $statuslist .= '<textarea id="replytext_'.$statusid.'" class="replytext" onkeyup="statusMax(this,300)" placeholder="write a comment here"></textarea><br><button id="replyBtn_'.$statusid.'" class="replyBtn" onclick="replyToStatus('.$statusid.',\''.$u.'\',\'replytext_'.$statusid.'\',this)">Reply</button>';	
	}
}
?>
<script>
	...
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			...
			if(datArray[0] == "post_ok"){
				...
				_("statusarea").innerHTML = '<div id="status_'+sid+'" class="status_boxes"><div><b>Posted by you just now:</b> <span id="sdb_'+sid+'"><a href="#" onclick="return false;" onmousedown="deleteStatus(\''+sid+'\',\'status_'+sid+'\');" title="DELETE THIS STATUS AND ITS REPLIES">delete status</a></span><br />'+data+'</div></div><textarea id="replytext_'+sid+'" class="replytext" onkeyup="statusMax(this,300)" placeholder="write a comment here"></textarea><br><button id="replyBtn_'+sid+'" onclick="replyToStatus('+sid+',\'<?php echo $u; ?>\',\'replytext_'+sid+'\',this)">Reply</button>'+currentHTML;
